...
import hscs.pub.service.IPubApplyResultsService;
...
public class TaskExecutionController extends BaseController {
    ...
    @Autowired
    private IPubApplyResultsService pubApplyResultsService;

    @Autowired
    private ITaskExecutionDetailService taskExecutionDetailService;

    private static final String AUTO_APPLY = "HSCS_AUTO_APPLY";
    ...
    @ResponseBody
    public TaskExecution queryOne(TaskExecution dto, HttpServletRequest request) {
        IRequest iRequest = createRequestContext(request);
        return iTaskExecutionService.selectByPrimaryKey(iRequest, dto);
    }
    ...
    @ResponseBody
    public ResponseData resultDownload(Long executionId, HttpServletRequest request, HttpServletResponse response) throws Exception {
        IRequest iRequest = createRequestContext(request);
        ...
        if (file.exists()) {
            ...
            List<TaskExecution> list = iTaskExecutionService.queryExecutionDetail(iRequest, execution);
            if (list != null && !list.isEmpty() && AUTO_APPLY.equals(list.get(0).getTaskDetail().getCode())) {
                ResponseData rd = new ResponseData();
                rd.setSuccess(true);
                rd.setCode(execution.getExecuteResultPath());
                return rd;
            }else{
